...
export const createDoc = async (db: Db, doc: { createdBy: string; folder: string; name: string; content?: any }) => {
    ...
    return db.collection('docs').insertOne({_id: nanoid(), ...doc, createdAt: new Date().toDateString() }).then(({ ops }) => ops[0])
}

export const updateOne = async (db: Db, id: string, updates: any) => {

    return db.collection('docs').updateOne({_id: id}, { $set: updates} )
}
